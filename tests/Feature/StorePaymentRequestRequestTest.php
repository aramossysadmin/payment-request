...
beforeEach(function () {
    ...
    $this->department->update(['authorizer_level_1_id' => $this->user->id]);

    $this->advanceType = PaymentType::factory()->create(['invoice_documents_mode' => 'disabled', 'additional_documents_mode' => 'optional']);
    $this->invoiceType = PaymentType::factory()->create(['invoice_documents_mode' => 'required', 'additional_documents_mode' => 'optional']);
...
test('invoice payment type requires pdf and xml documents', function () {
    $this->actingAs($this->user)
        ...
        ->assertSessionHasErrors('invoice_documents');
});
...
test('invoice payment type rejects more than two files', function () {
    ...
    $this->actingAs($this->user)
        ->post(route('payment-requests.store'), [
            ...$this->validData,
            'payment_type_id' => $this->invoiceType->id,
            'invoice_documents' => [$pdf, $xml, $extra],
        ])
        ->assertSessionHasErrors('invoice_documents');
});
