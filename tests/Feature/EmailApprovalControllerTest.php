...
test('notification email contains approval token url', function () {
    ...
    $this->department->update(['authorizer_level_1_id' => $this->authorizer->id]);
...
test('email renders approval button as primary action when token exists', function () {
    ...
    Notification::assertSentTo(
        ...
        function (PaymentRequestCreated $notification) {
            ...
            expect($mail->viewData['actionText'])->toBe('Autorizar / Rechazar Solicitud');
            expect($mail->viewData['actionUrl'])->toContain('/approval/');
            expect($mail->viewData['actionUrl'])->not->toContain('/admin/');
...
test('email renders filament link as primary action when token is null', function () {
    ...
    expect($mail->viewData['actionText'])->toBe('Ver Solicitud');
    expect($mail->viewData['actionUrl'])->toContain('/admin/payment-requests/');
});
